...
SELECT * FROM book WHERE pageCount = (SELECT MAX(pageCount) 
FROM book);


SELECT customer.custID, customer.fname,
MAX(DATEDIFF(transaction.return_date, transaction.issue_date)) AS 
max_borrow_duration
FROM customer
...
select book.bkID , title,author.fname from book left join author on 
...
SELECT customer.custID, customer.fname, COUNT(transaction.bkID)
AS 
books_borrowed
FROM customer 
JOIN transaction ON customer.custID = transaction.custID
